...
class PolypDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_name = name[:-4] + '_BE.tif'
        mask = cv2.imread(self.gt_root + '/' + mask_name, cv2.IMREAD_GRAYSCALE) / 255.0
...
def give_augmentations(config):
    augmentations = A.Compose([
        A.Normalize(),
        A.Resize(config.dataset.image_size, config.dataset.image_size, interpolation=cv2.INTER_NEAREST),
...
if __name__ == '__main__':
    image_root = '/workspace/Seg-Zhang/Esophagus-main/dataset/DEE/images'
    gt_root = '/workspace/Seg-Zhang/Esophagus-main/dataset/DEE/BE'
